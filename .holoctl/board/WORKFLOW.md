...
1. Get next ID: `holoctl board next-id`
2. Prepare ticket data as JSON.
3. Create: `holoctl board add '<json>'`
...
```bash
holoctl board stat                              # counts by status
holoctl board get TST-001               # single ticket (JSON)
holoctl board ls [--sprint X] [--status X]      # list with filters
          [--agent X] [--tag X] [pN]
holoctl board move TST-001 doing        # move + dual-write
holoctl board set TST-001 sprint s1     # update field
holoctl board add '<json>'                      # create ticket (auto-ID)
holoctl board next-id                           # next available ID
holoctl board rebuild-index                     # rebuild from .md files
```
