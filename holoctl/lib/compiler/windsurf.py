...
def compile_windsurf(project_root: Path, config: dict, dry_run: bool = False) -> dict:
    ...
    # Hooks → .windsurf/hooks.json (reuses cursor template if no windsurf-specific one)
    files.extend(hooks_emit.emit_windsurf(project_root, dry_run=dry_run))

    # MCP server config → .windsurf/mcp.json
    files.extend(mcp_emit.emit_windsurf(project_root, dry_run=dry_run))
