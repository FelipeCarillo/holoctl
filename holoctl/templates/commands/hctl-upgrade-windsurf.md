---
description: Upgrade holoctl in this workspace and sync templates, commands and tickets
---

**Default = execute** except Step 2 (install requires confirmation).

# Step 1 — Diagnostic

`hctl upgrade --check`. Shows `workspace_version`, `installed_version`, and a CHANGELOG slice between them. If equal, exit "already in sync" and stop.
...
`hctl upgrade` (no flags). Orchestrates: `sync --agents` → `compile` per target → `board rebuild-index` → `doctor` → bumps `holoctlVersion`. Show the full output.

# Step 4 — Overview

`hctl overview` and show the output.
