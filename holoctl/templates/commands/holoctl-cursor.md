...
Run `hctl doctor`. "No .holoctl/ found" → Step 2. Otherwise → Step 5.

# Step 2 — Init

Infer name + prefix from `cwd.name` and run `hctl init --name "<n>" --prefix "<P>"` (or `hctl init` if obvious). Auto-compiles cursor target.
...
**4.1 Sub-repos (✋ ASK once, aggregated)**: if multi-project, ask one question listing all candidates. For each approved: `hctl repo add ./<path> --name <n> --description "<one-line>"`. Single project → skip silently.
...
**4.4** `hctl compile --target cursor`.
