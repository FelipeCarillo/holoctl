...
def init_cmd(
    ...
    """Initialize .projctl/ in the current directory."""
    cwd = Path.cwd()
    projctl_dir = cwd / ".projctl"

    if (projctl_dir / "config.json").exists():
        console.print("[yellow].projctl/ already exists in this directory.[/yellow]")
        raise typer.Exit(1)
    ...
    (cwd / ".projctl" / "board" / "index.json").write_text(
        json.dumps(index_data, indent="\t") + "\n", encoding="utf-8"
    )
    (cwd / ".projctl" / "activity.jsonl").write_text("", encoding="utf-8")
    ...
    console.print(f"  [green]✓ .projctl/ initialized successfully.[/green]\n")
    console.print("  Next steps:")
    console.print(f"    [dim]$[/dim] projctl board add '{{\"title\":\"My first ticket\",\"agent\":\"developer\"}}'")
    console.print(f"    [dim]$[/dim] projctl serve")
    console.print(f"    [dim]$[/dim] projctl compile --target {target_list[0]}")
    console.print("")
