...
def _version_file(dest_dir: Path) -> Path:
    return dest_dir / ".projctl_version"
...
def setup_global_cmd(
    ...
    """Install /projctl slash command globally for AI coding assistants."""
    if check:
        versions = check_installed_versions()
        if not versions:
            console.print("\n  [dim]/projctl is not installed in any known location.[/dim]\n")
            return
        console.print("\n  [bold]projctl setup-global --check[/bold]\n")
        for v in versions:
    ...
    console.print("\n  [bold]projctl setup-global[/bold]\n")
    for r in results:
        if r["status"] == "ok":
            ...
            console.print(f"  [yellow]?[/yellow] [bold]{r.get('label', r['target'])}[/bold]: template not found — reinstall projctl")
        else:
    ...
    if ok_count > 0 and not dry_run:
        ...
        console.print(f"\n  [green]/projctl is now available in {ok_count} tool{s}[/green]\n")
        if "claude" in target_list:
            console.print("[dim]  Open Claude Code and type /projctl to get started.[/dim]\n")
