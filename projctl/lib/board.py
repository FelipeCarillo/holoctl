...
class Board:
    def __init__(self, project_root: Path, config: dict) -> None:
        ...
        self._config = config
        self._board_dir = project_root / ".projctl" / "board"
    ...
    def ls(self, filters: dict | None = None) -> list[dict]:
        ...
        if f.get("scope"):
            tickets = [t for t in tickets if t.get("scope") == f["scope"]]
...
def _log_activity(project_root: Path, event: dict) -> None:
    ...
    log_path = project_root / ".projctl" / "activity.jsonl"
