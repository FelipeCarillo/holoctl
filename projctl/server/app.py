...
app = FastAPI(title="projctl dashboard", docs_url=None, redoc_url=None)
...
def _get_projects() -> list[dict]:
    ...
    for p in list_workspace():
        try:
            ...
            stats = board.stat()
            agents_dir = Path(p["path"]) / ".projctl" / "agents"
...
def _read_commands(project_path: Path) -> list[dict]:
    d = project_path / ".projctl" / "commands"
    if not d.exists():
        return []
    result = []
    for f in sorted(d.glob("*.md")):
...
def _read_context_docs(project_path: Path) -> list[dict]:
    d = project_path / ".projctl" / "context"
    if not d.exists():
        return []
    items = []
...
def _sidebar(projects: list[dict], current_alias: str = "", current_tab: str = "") -> str:
    ...
    return f"""
<div class="sidebar-header"><a href="/" class="sidebar-logo">projctl</a></div>
...
<a href="/project/{p['alias']}/board" class="project-card">
  ...
  <div class="project-card-targets">{' '.join(p.get('targets', []))}</div>
    ...
    if not projects:
        cards = '<div class="empty-state"><h3>No projects yet</h3><p>Run <code>projctl init</code> in a directory to get started.</p></div>'
...
def project_board(alias: str):
    ...
    return _render(
        ...
        breadcrumbs=[{"label": "projctl", "href": "/"}, {"label": project["name"], "href": f"/project/{alias}/board"}, {"label": "Board"}],
...
def project_agents(alias: str):
    ...
    return _render(
        ...
        breadcrumbs=[{"label": "projctl", "href": "/"}, {"label": project["name"], "href": f"/project/{alias}/board"}, {"label": "Agents"}],
        tabs=_PROJECT_TABS, tab_base=f"/project/{alias}",
    )
...
def project_commands(alias: str):
    ...
    return _render(
        ...
        breadcrumbs=[{"label": "projctl", "href": "/"}, {"label": project["name"], "href": f"/project/{alias}/board"}, {"label": "Commands"}],
        tabs=_PROJECT_TABS, tab_base=f"/project/{alias}",
    )
...
def project_context(alias: str):
    ...
    return _render(
        ...
        breadcrumbs=[{"label": "projctl", "href": "/"}, {"label": project["name"], "href": f"/project/{alias}/board"}, {"label": "Context"}],
        tabs=_PROJECT_TABS, tab_base=f"/project/{alias}",
    )
...
def project_ticket(alias: str, ticket_id: str):
    ...
    ticket_file = Path(project["path"]) / ".projctl" / "board" / ticket["file"]
    ...
    return _render(
        ...
        current_alias=alias, current_tab="board",
        breadcrumbs=[{"label": "projctl", "href": "/"}, {"label": project["name"], "href": f"/project/{alias}/board"}, {"label": "Board", "href": f"/project/{alias}/board"}, {"label": ticket_id}],
        tabs=_PROJECT_TABS, tab_base=f"/project/{alias}",
    )
...
def project_repos(alias: str):
    ...
    return _render(
        ...
        breadcrumbs=[{"label": "projctl", "href": "/"}, {"label": project["name"], "href": f"/project/{alias}/board"}, {"label": "Repos"}],
        tabs=_PROJECT_TABS, tab_base=f"/project/{alias}",
    )
...
def project_files(alias: str):
    ...
    return _render(
        ...
        breadcrumbs=[{"label": "projctl", "href": "/"}, {"label": project["name"], "href": f"/project/{alias}/board"}, {"label": "Files"}],
        tabs=_PROJECT_TABS, tab_base=f"/project/{alias}",
    )
...
def api_board(alias: str):
    ...
    index_path = Path(project["path"]) / ".projctl" / "board" / "index.json"
