...
def workspace_add(
    ...
    if not (resolved / ".projhub" / "config.json").exists():
        console.print(f"[red]No .projhub/ found at {resolved}. Run `projhub init` first.[/red]")
        raise typer.Exit(1)
...
def workspace_list():
    ...
    if not projects:
        console.print("[dim]No projects registered. Run `projhub init` in a project directory.[/dim]")
        return
    for p in projects:
        exists = (Path(p["path"]) / ".projhub" / "config.json").exists()
