(function () {
  ...
  function initTheme() {
    const saved = localStorage.getItem('projhub-theme');
  ...
  window.__toggleTheme = function () {
    ...
    localStorage.setItem('projhub-theme', next);
  ...
  // ── Scope filter (board page) ──

  window.__filterScope = function (btn, scope) {
    document.querySelectorAll('.scope-btn').forEach(b => b.classList.remove('active'));
    btn.classList.add('active');
    document.querySelectorAll('.kanban-card').forEach(card => {
      const cardScope = card.getAttribute('data-scope') || '';
      card.style.display = (scope === '' || cardScope === scope) ? '' : 'none';
    });
  };
