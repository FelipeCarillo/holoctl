...
```bash
projhub init --name "<inferred>" --prefix "<INFERRED>"
```

(Or `projhub init` with no flags if the inference is obvious — the CLI derives both from `cwd.name`.) This auto-runs `projhub compile --target claude` and `projhub setup-global`.
...
- **`.projhub/context/objective.md`**: What / Why / Success criteria, derived from README. If success criteria can't be inferred, leave the bullets blank with a comment `<!-- TBD -->`.
- **`.projhub/context/architecture.md`**: Tech stack (real frameworks/libs from package files), Structure (real top-level layout), Key patterns (only patterns visible in code), Boundaries.
- **`.projhub/context/conventions.md`**: derived from real lint/format configs. For dimensions without config, write `(not enforced)` instead of guessing.
- **`.projhub/instructions.md`**: fill Identity (1-2 sentences from objective.md "What") and Folder map (real top-level dirs).
